...
export const editDescription = [
  ...
  "A mitigation or an assumption is added linked to at least one threat. A record's references are its threat links, and an assumption that applies to the model also has its model link, which a replace keeps. An edit that takes a record from one or more references to none (removing the threat that was its last reference, unlinking it, or a replace that leaves it no threat link) removes the record in the same edit. The result names under `culled`, once each, every record the file held before the batch that an edit of the batch culled, even where a later edit adds it back. A record removed by `remove_mitigation` or `remove_assumption` is not named there, and neither is one the batch itself added.",
