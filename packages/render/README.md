...
`@saerskriven/model/fixtures`, so a further diagram joins every check over
them by being added there. The Typst document and the Markdown register are
written from `test-data/two-diagrams.model.json`. Cached tests write no snapshot, so a missing golden
