...
- A threat is **open**, with an undecided severity, where the surface is
  designed and its issue is named but nothing is built to assess.
...
spec), so a further native model joins them by being added to each. The
render and canvas suites read the model the formats suite derives from it,
`test-data/saerskriven.model.json` ([test-data](../test-data/README.md)).
...
names and badges of the diagram, so moving an element toward a boundary can put
a name over a line. Re-render and look after any move.
