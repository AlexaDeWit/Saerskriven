...
describe('validate', () => {
  it.each([
    {
      named: 'reads a Threat Dragon file and counts what the model holds',
      read: () => validate(testDataPath('threat-dragon/feature-complete.json')),
      expected: {
        code: 0,
    ...
    {
      named:
        'reads a native file of two diagrams and counts what the model holds',
      read: () => validate(testDataPath('saerskriven/feature-complete.yaml')),
      expected: {
        code: 0,
    ...
    {
      named: 'warns about what a read dropped, and still succeeds',
      read: () => validated('undeclared.yaml', undeclaredKeyYaml),
      expected: {
        code: 0,
  ...
  it('lists the formats tried where none claimed the text', () => {
    const outcome = validated('unclaimed.yaml', unclaimedYaml);
    expect(outcome).toMatchObject({ code: 1, out: '' });
    expect(outcome.err).toContain('threat-dragon');
    expect(outcome.err).toContain('saerskriven-yaml');
  });
